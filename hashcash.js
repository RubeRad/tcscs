 ...
 */
function hashcashv1_create(updateCallback, completeCallback, str, bits) {
  setTimeout("hashcashv1_create_hidden('"+ updateCallback+"','"+completeCallback+"','"+str+"','"+ bits +"');", 100);
}

function hashcashv1_create_hidden(updateCallback, completeCallback, str, bits) {
...
//      alert(completeCallback +"('"+ hashcashv1_appptr.create_loop +"{}"+ str+","+extn+","+bits+")" +"');");
      ...
      eval(completeCallback +"('"+ hashcashv1_appptr.create_loop(""+str,""+bits) +"');");
      return;
    }
  }

  // fall back on JavaScript... :(
  eval(completeCallback +"('"+ hashcashv1_create_loop(updateCallback, str,bits) +"');");
}
...
function hashcashv1_create_loop(updateCallback, str, bits) {
  d = new Date();
  ...
  var searchString = "1:"+ bits +":"+ year + month + day + hour + min +":"+ str +"::"+ random +":";
